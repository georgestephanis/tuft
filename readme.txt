...
When [Alpaca Issue Tracker](https://wordpress.org/plugins/alpaca-issue-tracker/) is active alongside Design Feedback:

* Each submission creates a matching `alpaca_issue` in your board's default (lowest-score) column.
* Issues are tagged with the submitter's browser and the type "Design Feedback" for easy filtering.
* The Design Feedback admin menu moves under the **Project Board** submenu, so the Alpaca board becomes the primary triage interface.
* Click any row in the Design Feedback list to open the full detail view including the screenshot.

== Installation ==

1. Upload the `design-feedback` folder to `wp-content/plugins/`.
2. Activate **Design Feedback** through the **Plugins** menu in WordPress.
...
In the WordPress database as a custom post type (`design_feedback`). You can review them under **Design Feedback** in the admin menu, or under **Project Board → Design Feedback** if Alpaca Issue Tracker is active.
...
Yes. Hook into the `df_feedback_submitted` action, which fires after a submission is fully saved:
...
With the default configuration, no — the submission endpoint is open to unauthenticated requests. Before deploying to a public site, change the `permission_callback` in `includes/class-df-rest-controller.php` to require a capability:
...
4. The Design Feedback admin list table with screenshot thumbnails.
